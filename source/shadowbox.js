...
(function (window, undefined) {
    ...
    shadowbox.options = {
        ...
        // Automatically play movies when they load.
        autoPlay: true,
    ...
    /**
     * The HTML markup Shadowbox uses.
     */
    shadowbox.markup = "" +
    '<div id="shadowbox">' +
        ...
        '<div id="sb-wrapper">' +
            '<div id="sb-body">' +
                ...
                '<div id="sb-cover"></div>' +
            '</div>' +
            '<a id="sb-close" href="#">Close</a>' +
            '<a id="sb-next" href="#">Next</a>' +
            '<a id="sb-prev" href="#">Previous</a>' +
        '</div>' +
    '</div>';
     ...
     */
    function register(player, fileExts) {
        fileExts = fileExts || [];

        if (!Array.isArray(fileExts)) {
            fileExts = [fileExts];
        }

        for (var i = 0, len = fileExts.length; i < len; ++i) {
            shadowbox.players[fileExts[i]] = player;
        }
    }
    ...
    /**
     * Appends Shadowbox' markup to the DOM.
     */
    function initialize() {
        if (initialized) {
            return;
        ...
        appendHtml(document.body, shadowbox.markup);

        container = get("shadowbox");
        overlay = get("sb-overlay");
        wrapper = get("sb-wrapper");
        cover = get("sb-cover");
        content = get("sb-content");
        ...
        addEvent(get("sb-close"), "click", clickCanceler(close));
        addEvent(get("sb-next"), "click", clickCanceler(next));
        addEvent(get("sb-prev"), "click", clickCanceler(previous));
    ...
    function clickCanceler(callback) {
        return function (e) {
            e.preventDefault();
            callback();
            return false;
     ...
     *   - formats      (video only) Extension name/URL pairs of alternate URL's
     *                  for the video. Recognizes "mp4", "ogg", "webm" and "flv"
    ...
    function makePlayer(obj) {
        ...
        if (obj && typeof obj.url == "string") {
            var id = "sb-player-" + String(++playerId),
                playerFn;
    ...
    /**
     * Quick and dirty way to parse JSON strings.
     */
    ...
    /**
     * Creates a new element with the given tagName.
     */
    ...
    function getStyle(element, style) {
        ...
        if (!supportsOpacity && style == "opacity" && element.currentStyle) {
            if (reOpacity.test(element.currentStyle.filter || "")) {
